...
pub async fn list(pool: &PgPool, user_id: UserId) -> Result<Vec<Thought>> {
    let thoughts =
...
pub async fn get(pool: &PgPool, id: ThoughtId, user_id: UserId) -> Result<Option<Thought>> {
    let thought =
...
pub async fn create(pool: &PgPool, user_id: UserId, input: &CreateThought) -> Result<Thought> {
    ...
    let thought = sqlx::query_as::<_, Thought>(
        r#"
        INSERT INTO thoughts (user_id, title, position)
        VALUES ($1, $2, $3)
        RETURNING *
        "#,
    )
...
pub async fn delete(pool: &PgPool, id: ThoughtId, user_id: UserId) -> Result<bool> {
